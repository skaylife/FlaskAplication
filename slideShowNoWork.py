...
class App():
    ...
    def _draw_settings_screen(self):
        ...
        for key in self.settings:
            if key != "default":
                ...
                ttk.Button(self.settings_screen, text="Delete", command=lambda: self._delete_setting(key)).grid(row=row,
                                                                                                                column=2)
                row += 1
    ...
    def json_settings(self):
        json_string = '{'
        for key in self.settings:
            json_string += '"{}":{{"timer":{}, "name":"{}"}},'.format(key, self.settings[key]["timer"].get(),
                                                                      self.settings[key]["name"].get())
